...
def read_modes(list1, val1, pos_val2):
    if val1 == "1":
        return pos_val2
    if val1 == "0":
        return list1[pos_val2]

def new_intcode(list1):
    i = 0

    while list1[i] != 99 and i <= len(list1):
        
        opcode = str(list1[i])
        opcode = "0000"[len(opcode):] + opcode
            
        if opcode[-1] == "1":
            list1[read_modes(list1, "0", i+3)] = list1[read_modes(list1, opcode[1], i+1)] + list1[read_modes(list1, opcode[0], i+2)]
            i = i + 4
            ...
            list1[list1[i+1]] = int(input('Initial Input?: '))
            i = i + 2
        
        elif opcode[-1] == "4":
            output_ = list1[read_modes(list1, opcode[1], i+1)] #list1[list1[i+1]]
            print("OUTPUT", output_)
            i = i + 2

def new_intcode_2(list1):
    i = 0
    ...
    while list1[i] != 99 and i <= len(list1):
        ...
        if opcode[-1] == "3":
            ...
            i = list1[read_modes(list1, opcode[0], i+2)] if list1[read_modes(list1, opcode[1], i+1)] != 0 else i + 3
        
        elif opcode[-1] == "6":
            i = list1[read_modes(list1, opcode[0], i+2)] if list1[read_modes(list1, opcode[1], i+1)] == 0 else i + 3
            
        elif opcode[-1] == "7":
            list1[read_modes(list1, "0", i+3)] = 1 if list1[read_modes(list1, opcode[1], i+1)] < list1[read_modes(list1, opcode[0], i+2)] else 0
            i = i + 4
