...
def intcode(list1):
    ...
    while list1[i] != 99 and i <= len(list1):
        ...
        opcode = str(list1[i])
        opcode = "00000"[len(opcode):] + opcode
            
        if opcode[-1] == "1":
            list1[read_modes(opcode[0], i+3)] = list1[read_modes(opcode[2], i+1)] + list1[read_modes(opcode[1], i+2)]
            i = i + 4
            ...
            output_ = list1[read_modes(opcode[2], i+1)] #list1[list1[i+1]]
...
def intcode_2(list1):
    ...
    while list1[i] != 99 and i <= len(list1):
        ...
        if opcode[-1] == "1":
            ...
            i = list1[read_modes(opcode[1], i+2)] if list1[read_modes(opcode[2], i+1)] != 0 else i + 3
        
        elif opcode[-1] == "6":
            i = list1[read_modes(opcode[1], i+2)] if list1[read_modes(opcode[2], i+1)] == 0 else i + 3
            
        elif opcode[-1] == "7":
            list1[read_modes(opcode[0], i+3)] = 1 if list1[read_modes(opcode[2], i+1)] < list1[read_modes(opcode[1], i+2)] else 0
            i = i + 4
